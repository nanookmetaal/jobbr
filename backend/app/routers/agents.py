...
async def run_analysis(
    ...
    # Rate limit: max 5 analyses per profile per day
    today_start = datetime.now(timezone.utc).replace(hour=0, minute=0, second=0, microsecond=0)
    count_result = await db.execute(
        select(func.count()).select_from(AgentAnalysis).where(
            AgentAnalysis.profile_id == profile.id,
            AgentAnalysis.created_at >= today_start,
        )
    )
    if count_result.scalar() >= 5:
        raise HTTPException(status_code=429, detail="Daily analysis limit reached (5 per day). Try again tomorrow.")

    # Only re-analyse if the profile has changed since the last analysis
    last_analysis_result = await db.execute(
        select(AgentAnalysis.created_at)
        .where(AgentAnalysis.profile_id == profile.id)
        .order_by(AgentAnalysis.created_at.desc())
        .limit(1)
    )
    last_analyzed_at = last_analysis_result.scalar_one_or_none()
    if last_analyzed_at and profile.updated_at <= last_analyzed_at:
        raise HTTPException(
            status_code=400,
            detail="Profile hasn't changed since the last analysis. Update your profile first."
        )
